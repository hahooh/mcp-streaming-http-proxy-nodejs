...
class MCPStreamingHTTPProxy {
  constructor(streamingHttpUrl, apiKey = null, serverName = "My MCP Server") {
    this.streamingHttpUrl = streamingHttpUrl;
    this.apiKey = apiKey;
  ...
  async initialize() {
    // Validate the StreamingHTTP URL format
    try {
      const url = new URL(this.streamingHttpUrl);
      console.error(
        `Connecting to StreamingHTTP endpoint: ${this.streamingHttpUrl}`
      );
      console.error(
        `URL components - protocol: ${url.protocol}, host: ${url.host}, pathname: ${url.pathname}`
      );
    } catch (error) {
      throw new Error(
        `Invalid StreamingHTTP URL format: ${this.streamingHttpUrl} - ${error.message}`
      );
    }

    // Create the client to connect to the remote StreamingHTTP server
    this.client = new Client(
      {
        name: `${this.serverName}-proxy-client`,
        version: "1.0.0",
      },
      {
        capabilities: {},
    ...
    // Create the StreamingHTTP transport for the client
    // The SSEClientTransport expects a URL object, not a string
    console.error(
      `Creating SSEClientTransport with URL: ${this.streamingHttpUrl}`
    );
    const urlObject = new URL(this.streamingHttpUrl);
    ...
    // Connect client to the remote StreamingHTTP server with better error handling
    console.error(`Attempting to connect to StreamingHTTP server...`);
    try {
      await this.client.connect(clientTransport);
      console.error(`Successfully connected to StreamingHTTP server`);
    } catch (error) {
      console.error(`Failed to connect to StreamingHTTP server:`, error);
      if (error.stack) {
  ...
  setupProxyHandlers() {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      try {
        ...
        if (Array.isArray(result)) {
          ...
          console.error(
            `Client returned wrapped format with ${result.tools.length} tools`
          );
          return result;
        } else {
          console.error(
            "Unexpected result format from client.listTools():",
            typeof result,
            result
          );
          return { tools: [] };
    ...
    this.server.setRequestHandler(ListResourcesRequestSchema, async () => {
      try {
        ...
        console.error(
          "Raw client listResources() result:",
          JSON.stringify(result, null, 2)
        );
        ...
        if (Array.isArray(result)) {
          ...
          console.error(
            `Client returned wrapped format with ${result.resources.length} resources`
          );
          return result;
        } else {
          console.error(
            "Unexpected result format from client.listResources():",
            typeof result,
            result
          );
          return { resources: [] };
    ...
    this.server.setRequestHandler(
      ...
      async () => {
        try {
          console.error(
            "Attempting to list resource templates from remote server..."
          );
          const result = await this.client.listResourceTemplates();
          console.error(
            "Raw client listResourceTemplates() result:",
            JSON.stringify(result, null, 2)
          );

          if (Array.isArray(result)) {
            console.error(
              `Successfully retrieved ${result.length} resource templates`
            );
            ...
            console.error(
              `Client returned wrapped format with ${result.resourceTemplates.length} resource templates`
            );
            return result;
          } else {
            console.error(
              "Unexpected result format from client.listResourceTemplates():",
              typeof result,
              result
            );
    ...
    this.server.setRequestHandler(ListPromptsRequestSchema, async () => {
      try {
        ...
        if (Array.isArray(result)) {
          ...
          console.error(
            `Client returned wrapped format with ${result.prompts.length} prompts`
          );
          return result;
        } else {
          console.error(
            "Unexpected result format from client.listPrompts():",
            typeof result,
            result
          );
          return { prompts: [] };
    ...
    this.server.setRequestHandler(GetPromptRequestSchema, async (request) => {
      try {
        console.error(
          `Getting prompt: ${request.params.name} with args:`,
          request.params.arguments
        );
  ...
  async run() {
    ...
    console.error(
      `MCP StreamingHTTP Proxy "${this.serverName}" running - connecting to ${this.streamingHttpUrl}`
    );
  }
...
async function main() {
  ...
  console.error(`Resolved StreamingHTTP URL: "${streamingHTTPUrl}"`);
  ...
  const proxy = new MCPStreamingHTTPProxy(streamingHTTPUrl, apiKey, serverName);
